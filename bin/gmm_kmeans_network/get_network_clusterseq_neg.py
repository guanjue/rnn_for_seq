...
network_pos, network_superstate_pos, pred_cluster_pos, pred_supercluster_pos, edge_num_pos, t_s_pos, t_s_super_pos = generate_net(labels_gmm_kmeans0_pos, sequence0_pos, h_pred0_pos)
#########################
### for substate network
network_matrix_pos = netdict_seq2char_netmatrix(network_pos, seq, edge_num, t_s, labels_gmm_kmeans0, labels_gmm_kmeans0_pos)
print('network_matrix_pos_k.shape: '+str(network_matrix_pos.shape))
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_matrix_pos,'network_table_pos_kmeans/network_table_pos.txt')
################################################################################################
### get significant substate network edge
network_matrix_thresh_pos, network_matrix_thresh_noself_pos  = extract_enriched_edges(network_matrix_pos, np.array(network_matrix[:,5],dtype=float), 0.99, 'network_table_pos_kmeans/network_enrichment_hist_state_pos.pdf')
### write matrix txt
write2d_array(network_matrix_thresh_pos,'network_table_pos_kmeans/network_matrix_thresh_pos.txt')
write2d_array(network_matrix_thresh_noself_pos,'network_table_pos_kmeans/network_matrix_thresh_noself_pos.txt')
################################################################################################
...
#########################
### for superstate network
network_superstate_matrix_pos = netdict_seq2char_netmatrix(network_superstate_pos, seq, edge_num, t_s_super, labels_gmm_kmeans0, labels_gmm_kmeans0_pos)
print('network_superstate_matrix_pos.shape: '+str(network_superstate_matrix_pos.shape))
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_superstate_matrix_pos,'network_table_pos_kmeans/network_superstate_table_pos.txt')
################################################################################################
### get significant superstate network edge
network_superstate_matrix_thresh_pos, network_superstate_matrix_thresh_noself_pos  = extract_enriched_edges(network_superstate_matrix_pos, np.array(network_superstate_matrix[:,5],dtype=float), 0.99, 'network_table_pos_kmeans/network_superstate_enrichment_hist_state_pos.pdf')
### write matrix txt
write2d_array(network_superstate_matrix_thresh_pos,'network_table_pos_kmeans/network_superstate_matrix_thresh_pos.txt')
write2d_array(network_superstate_matrix_thresh_noself_pos,'network_table_pos_kmeans/network_superstate_matrix_thresh_noself_pos.txt')
################################################################################################
...
print('network_matrix_pos_k.shape: '+str(network_matrix_pos.shape))
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_matrix_pos,'network_table_pos_kmeans/network_table_neg.txt')
################################################################################################
### get significant substate network edge
network_matrix_thresh_pos, network_matrix_thresh_noself_pos  = extract_enriched_edges(network_matrix_pos, np.array(network_matrix[:,5],dtype=float), 0.99, 'network_table_pos_kmeans/network_enrichment_hist_state_pos.pdf')
### write matrix txt
write2d_array(network_matrix_thresh_pos,'network_table_pos_kmeans/network_matrix_thresh_neg.txt')
write2d_array(network_matrix_thresh_noself_pos,'network_table_pos_kmeans/network_matrix_thresh_noself_neg.txt')
################################################################################################
...
print('network_superstate_matrix_pos.shape: '+str(network_superstate_matrix_pos.shape))
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_superstate_matrix_pos,'network_table_pos_kmeans/network_superstate_table_neg.txt')
################################################################################################
### get significant superstate network edge
network_superstate_matrix_thresh_pos, network_superstate_matrix_thresh_noself_pos  = extract_enriched_edges(network_superstate_matrix_pos, np.array(network_superstate_matrix[:,5],dtype=float), 0.99, 'network_table_pos_kmeans/network_superstate_enrichment_hist_state_pos.pdf')
### write matrix txt
write2d_array(network_superstate_matrix_thresh_pos,'network_table_pos_kmeans/network_superstate_matrix_thresh_neg.txt')
write2d_array(network_superstate_matrix_thresh_noself_pos,'network_table_pos_kmeans/network_superstate_matrix_thresh_noself_neg.txt')
################################################################################################
...
def extract_enriched_edges(netmatrix, net_enrichment_array, significance, enrichement_hist_filename):
	import matplotlib.pyplot as plt
	from scipy import stats
	print('network_enrichment analysis:')
	### extract enrichment array
	net_enrichment_forhist = np.log(net_enrichment_array) ### use all sample enrichment to calculate enrichment
	### get significant stat	
	z_score = stats.norm.ppf(significance)
...
### before sample clustering
network_pos, network_superstate_pos, pred_cluster_pos, pred_supercluster_pos, edge_num_pos, t_s_pos, t_s_super_pos = generate_net(labels_gmm_kmeans0[pos_neg], sequence0, h_pred0)
network, network_superstate, pred_cluster, pred_supercluster, edge_num, t_s, t_s_super = generate_net(labels_gmm_kmeans0, sequence0, h_pred0)
################################################################################################
### for substate network
#########################
seq = ['A','C','G','T']
### for substate network
network_matrix_pos = netdict_seq2char_netmatrix(network_pos, seq, edge_num, t_s, labels_gmm_kmeans0, labels_gmm_kmeans0[pos_neg])
network_matrix = netdict_seq2char_netmatrix(network, seq, edge_num, t_s, labels_gmm_kmeans0, labels_gmm_kmeans0)
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_matrix,'network_table.txt')
################################################################################################
### get significant substate network edge
network_matrix_thresh, network_matrix_thresh_noself  = extract_enriched_edges(network_matrix_pos, np.array(network_matrix_pos[network_matrix_pos[:,0]!=network_matrix_pos[:,2],5],dtype=float), 0.99, 'network_enrichment_hist_state.pdf')
### write matrix txt
write2d_array(network_matrix_thresh,'network_matrix_thresh.txt')
write2d_array(network_matrix_thresh_noself,'network_matrix_thresh_noself.txt')
################################################################################################
...
seq = ['A','C','G','T']
### for superstate network
network_superstate_matrix_pos = netdict_seq2char_netmatrix(network_superstate_pos, seq, edge_num, t_s_super, labels_gmm_kmeans0, labels_gmm_kmeans0[pos_neg])
network_superstate_matrix = netdict_seq2char_netmatrix(network_superstate, seq, edge_num, t_s_super, labels_gmm_kmeans0, labels_gmm_kmeans0)
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_superstate_matrix,'network_superstate_table.txt')
#network_matrix = network_matrix[np.argsort(np.array(network_matrix[:,3],dtype=int))]
write2d_array(network_superstate_matrix_pos,'network_superstate_matrix_pos_table.txt')
################################################################################################
### get significant superstate network edge
network_superstate_matrix_thresh, network_superstate_matrix_thresh_noself  = extract_enriched_edges(network_superstate_matrix_pos, np.array(network_superstate_matrix_pos[network_superstate_matrix_pos[:,0]!=network_superstate_matrix_pos[:,2],5],dtype=float), 0.99, 'network_superstate_enrichment_hist_state.pdf')
### write matrix txt
write2d_array(network_superstate_matrix_thresh,'network_superstate_matrix_thresh.txt')
write2d_array(network_superstate_matrix_thresh_noself,'network_superstate_matrix_thresh_noself.txt')
################################################################################################
