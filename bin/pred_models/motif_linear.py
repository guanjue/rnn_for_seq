...
def weight_motifscan_variable():
	...
	[0.000000,0.000000,1.000000,0.000000],
	[0.000000,0.309586,0.000000,0.690414]
	]],
	[[
	[0.690414,0.000000,0.309586,0.000000],
	...
	### change to np array
	weight_motifscaner=np.array(weight_motifscaner)
	print('motif scanner')
	print(weight_motifscaner.shape)
	weight_motifscaner=np.transpose(weight_motifscaner,(2,1,3,0))
	weight_motifscaner=tf.constant(weight_motifscaner,dtype=tf.float32)
	print(weight_motifscaner.shape)
	return weight_motifscaner
...
W_conv1 = weight_motifscan_variable()
...
#print('h_pool1_wta.shape : sparse coding maxpooling depth' )
#print(tf.shape(h_pool1_wta))
h_pool1 = tf.nn.max_pool(h_pool1_wta, ksize=[1, filter1_max_pool_size, 1, 1],strides=[1, filter1_max_pool_size, 1, 1], padding='SAME')

h_pool_reshape = tf.reshape(h_pool1, [-1, pool_shape1 * filter1_size_out])
...
for i in range(iter_num):
	...
	if i%1000 == 0:
		...
		if 1==1:#accuracy_r2_test<=r2_test and accuracy_r2_train<=r2_train:
			saver = tf.train.Saver()
			save_path = saver.save(sess, "trained_linear_model.ckpt")
